...
<!--            <div class="flex justify-between items-center"><span class="font-light text-gray-600">{{ this.date }}</span><a href="#"-->
<!--                                              class="px-2 py-1 bg-gray-600 text-gray-100 font-bold rounded hover:bg-gray-500">Laravel</a>-->
<!--            </div>-->
            <div class="flex">
            <div class="flex" v-for="(tag,key) in this.tags_titles">
                <span class="bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{{ tag }}</span>

            </div>
        </div>
            <div class="mt-2"><a href="#" class="text-2xl text-gray-700 font-bold hover:underline">{{ this.title }}</a>
                <p class="mt-2 text-gray-600">{{ this.short_description }}</p>
            </div>
            <div   @click="this.open_post(this.id)" class="flex justify-between items-center mt-4"><button href="#"
                                                                   class="text-blue-500 hover:underline" >Read</button>
